...
export type MiCasaWebSocket = {
  ...
  addEventListener(type: "open" | "error" | "close", listener: EmptyListener): void;
...
function parseFrame(value: unknown): unknown[] {
  if (typeof value !== "string" || value.length > MAX_FRAME_BYTES) {
    fail("MICASA_REALTIME_FRAME_INVALID", "The relay returned an invalid frame.");
  }
  let frame: unknown;
  try {
...
async function sign(
  ...
  try {
    ...
    fail("MICASA_REALTIME_SIGNER_UNAVAILABLE", "The MiCasa signer is unavailable.");
  }
...
export class MiCasaRealtimeClient {
  ...
  async queryChannelHistory(
    ...
    return new Promise((resolve, reject) => {
      ...
      const subscriptionId =
        "micasa-" + crypto.randomUUID().replaceAll("-", "");
      let settled = false;
  ...
  async publishChannelMessage(
    ...
    if (
      mentions.length > 64 ||
      mentions.some((value) => !HEX_64.test(value))
    ) {
    ...
    const template: UnsignedNostrEvent = {
      ...
      created_at: Math.floor(
        (this.#options.now?.() ?? Date.now()) / 1_000,
      ),
      tags: [
        ["h", checkedChannel],
